...
test('returns winner', () => {
  ...
  utils.getWinner = (...args) => {
    utils.getWinner.mock.calls.push(arguments)
    return p2
  }
  ...
  expect(utils.getWinner.mock.length).toEqual(2)
  // add another assertion that every time it was called
  // it was called with the right arguments: 'Ken Wheeler', 'Kent C. Dodds'
  // (Hint #3)
  utils.getWinner.mock.forEach(call => {
   expect(call.arguments).toContain('Ken Wheeler') 
   expect(call.arguments).toContain('Kent C. Dodds')
  });
